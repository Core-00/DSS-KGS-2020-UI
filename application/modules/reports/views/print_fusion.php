    ...
    <div class="container">
        <div class="row mt-3 mb-5">
            <div class="col-md-12">
                ...
                <h6 class="text-center font-weight-bold" id="single-print-doc-name">Fusion Result</h6>
            </div>
        </div>
    ...
    $(document).ready(function(){
        $.ajax({
            ...
            success: function(response) {
                ...
                // ==========><>|Total Parameter|<><==========
                $("#single-print-total").empty().html(`${response.merge_result.sum_persen.length}`)
                // ==========><>|Total Parameter|<><==========

                // ==========><>|Total Not Zero|<><==========
                single_print_total_not_zero = response.merge_result.sum_persen.filter((data) => {
                ...
                // ==========><>|Total Zero|<><==========
                single_print_total_zero = response.merge_result.sum_persen.filter((data) => {
                    return parseInt(data) === 0
                })
                ...
                // ==========><>|Probability|<><==========
                response.merge_result.sum_biner.forEach((data) => {
                    // ==========><>|Highest Probability|<><==========
                ...
                // ==========><>|Highest Probability|<><==========
                single_print_highest_probability_parameter = response.merge_result.sum_biner.filter((data) => {
                ...
                // ==========><>|Lowest Probability|<><==========
                single_print_lowest_probability_parameter = response.merge_result.sum_biner.filter((data) => {
                ...
                var htmlHighest = ''
                var htmlLowest = ''
                response.merge_result.sum_persen.forEach((data, index) => {
                    html += `
                        <tr class="${parseInt(data) > 0 ? 'text-danger' : 'text-success'}">
                            <td align="center">${index + 1}</td>
                            <td>${response.per_table[0].key_biners[0].biner[index]}</td>
                            <td>${response.merge_result.sum_biner[index]}</td>
                            <td>${data}</td>
                        </tr>
                    `
                    if (parseFloat(response.merge_result.sum_biner[index]) === 1) {
                        htmlHighest += `
                            <li class="${parseInt(data) > 0 ? 'text-danger' : 'text-success'}">${response.per_table[0].key_biners[0].biner[index]}</li>
                        `
                    } else if (parseFloat(response.merge_result.sum_biner[index]) === 0) {
                        htmlLowest += `
                            <li class="${parseInt(data) > 0 ? 'text-danger' : 'text-success'}">${response.per_table[0].key_biners[0].biner[index]}</li>
                        `
                    }
